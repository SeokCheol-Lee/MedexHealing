...
import org.techtown.medexhealing.databinding.ActivityEsptouchBinding;
import org.techtown.medexhealing.EspTouchActivityAbs;
import org.techtown.medexhealing.EspTouchApp;
...
public class EspTouchActivity extends EspTouchActivityAbs {
    ...
    private ActivityEsptouchBinding Espbinding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Espbinding = ActivityEsptouchBinding.inflate(getLayoutInflater());
        setContentView(Espbinding.getRoot());
        Espbinding.confirmBtn.setOnClickListener(v -> executeEsptouch());

        Espbinding.cancelButton.setOnClickListener(v -> {
            showProgress(false);
            if (mTask != null) {
...
//        });
    ...
    private void showProgress(boolean show) {
        if (show) {
            Espbinding.content.setVisibility(View.INVISIBLE);
            Espbinding.progressView.setVisibility(View.VISIBLE);
        } else {
            Espbinding.content.setVisibility(View.VISIBLE);
            Espbinding.progressView.setVisibility(View.GONE);
        }
    }
    ...
    private void onWifiChanged() {
        ...
        Espbinding.apSsidText.setText(mSsid);
        Espbinding.apBssidText.setText(mBssid);
        Espbinding.messageView.setText(message);
        Espbinding.confirmBtn.setEnabled(confirmEnable);
    }

    private void executeEsptouch() {
        byte[] ssid = mSsidBytes == null ? ByteUtil.getBytesByString(this.mSsid)
                : mSsidBytes;
        CharSequence pwdStr = Espbinding.apPasswordEdit.getText();
        ...
        CharSequence devCountStr = Espbinding.deviceCountEdit.getText();
        byte[] deviceCount = devCountStr == null ? new byte[0] : devCountStr.toString().getBytes();
        byte[] broadcast = {(byte) (Espbinding.packageModeGroup.getCheckedRadioButtonId() == R.id.packageBroadcast
                ? 1 : 0)};
    ...
    private static class EsptouchAsyncTask4 extends AsyncTask<byte[], IEsptouchResult, List<IEsptouchResult>> {
        ...
        protected void onPreExecute() {
            EspTouchActivity activity = mActivity.get();
            activity.Espbinding.testResult.setText("");
        ...
        protected void onProgressUpdate(IEsptouchResult... values) {
            ...
            if (activity != null) {
                ...
                activity.Espbinding.testResult.append(String.format(
                        Locale.ENGLISH,
                        "%s,%s\n",
